...
export default function Header(props: {
  showInfo: any; theme: string; handleThemeChange: ChangeEventHandler<HTMLInputElement> | undefined;
}) {
  ...
  return (
    <div className="flex flex-row shrink-0 items-center justify-between rounded-[15px] w-full md:w-[638px] h-[60px] md:h-[84px] bg-[#f3f3f3] dark:bg-[#DADCE008] duration-150">
      <div className="w-1/3 px-4 mt-2">
        <button onClick={() => { props.showInfo(true) }}>
          <img src={props.theme === 'light' ? "/assets/icons/info.svg" : "/assets/icons/info_dark.svg"} alt="Info icon" className="hover:cursor-pointer" />
        </button>
      </div>
      ...
      <div className="flex flex-row justify-end items-center w-1/3 px-4 space-x-2">
        <button onClick={() => dispatchContext({ type: 'UPDATE_FINISHED' })}>
          <img src={props.theme === 'light' ? "/assets/icons/chart.svg" : "/assets/icons/chart_dark.svg"} alt="Chart icon" className="hover:cursor-pointer" />
        </button>
        <div className="flex flex-row items-center">
          <label>
            <input type="checkbox" checked={props.theme === 'light'} onChange={props.handleThemeChange} />
